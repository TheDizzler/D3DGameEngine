...
void SimpleShader::loadModel(Model * model) {


}

void SimpleShader::render(ID3D11DeviceContext * deviceContext, ID3D11Buffer * constantBuffers[]) {
}

HRESULT SimpleShader::initInputLayout(ID3D11Device * device, ID3D10Blob * vertexShaderBlob) {
	...
	return device->CreateInputLayout(vertexLayoutDesc, _countof(vertexLayoutDesc),
		vertexShaderBlob->GetBufferPointer(), vertexShaderBlob->GetBufferSize(), &layout);
}

HRESULT SimpleShader::initMatrixBuffer(ID3D11Device * device) {
	return E_NOTIMPL;
}


HRESULT SimpleShader::initSamplerState(ID3D11Device* device) {
...
HRESULT SimpleShader::initLightBuffer(ID3D11Device * device) {
	return E_NOTIMPL;
}

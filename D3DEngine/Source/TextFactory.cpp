...
void TextFactory::changeDevice(ID3D11Device *dev, ID3D11DeviceContext *devCon) {
...
void TextFactory::initialize(ID3D11Device *dev, ID3D11DeviceContext *devCon) {

	device = dev;
	deviceContext = devCon;
	if (reportError(FW1CreateFactory(FW1_VERSION, &pFW1Factory))) {
		MessageBox(NULL, L"Unable to create FontWrapperFactory", L"TextFactory ERROR", MB_OK);
		return;
	}

	if (reportError(pFW1Factory->CreateFontWrapper(device, L"Arial", &pFontWrapper))) {
		MessageBox(NULL, L"Unable to create FontWrapper", L"TextFactory ERROR", MB_OK);
		return;
	}

	if (reportError(pFontWrapper->GetDWriteFactory(&pDWriteFactory))) {
...
void altTextFormats() {
	...
	//if (reportError(pDWriteFactory->CreateTextFormat(
	//	L"Arial",
	//	NULL,
	...
	//if (reportError(pDWriteFactory->CreateTextLayout(
	//	str1,
	...
	//// No word wrapping
	//if (reportError(pTextLayout1->SetWordWrapping(DWRITE_WORD_WRAPPING_NO_WRAP))) {
	//	MessageBox(NULL, L"Unable to SetWordWrapping", L"TextFactory ERROR", MB_OK);
	//	return;
	//}
	//if (reportError(pTextLayout1->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_CENTER))) {
